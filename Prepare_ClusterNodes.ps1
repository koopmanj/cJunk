...
for ($i = 1; $i -lt 3; $i++)
{ 
    New-VirtualMachine -VMName w2k16-cluster$i -ImagesLocation F:\Images -VMsLocation J:\Hyper-V -VirtualSwitchName External -ProcessorCount 1 -Generation 2 -Memory 2048000 -Verbose    
    Start-VM -VMName w2k16-cluster$i
}
...
(Get-VM).where({$_.state -eq 'running'}).where({$_.name -like '*cluster*'}).foreach({
        Invoke-Command -ComputerName $PSItem.name  -ArgumentList $LocalCredentials -ScriptBlock {`
        param(
            $LocalCredentials
        )
